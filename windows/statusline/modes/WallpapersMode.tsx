...
const wallpapers = exec(
  `bash -c "find -L ~/.wallpapers/ -iname '*.png' -o -iname '*.jpg' -o -iname '*.jpeg'"`,
)
  .split("\n")
  .map((path) => ({
    name: path.split("/").pop(),
  ...
  return (
    <MenuMode
      ...
      onEnter={(selectedWallpaper) => {
        execAsync(`
            ~/.colors/colorizer.bash -w ${selectedWallpaper.path} -M
        `);
      }}
    >
      {queriedWallpapers((wallpapers) =>
        wallpapers.map((wallpaper) => (
          <box
            className={selectedWallpaper((selectedCmd) =>
              selectedCmd.name === wallpaper.name ? "selected item" : "item",
            )}
          >
            <label className="name" label={wallpaper.name} truncate={true} />
          </box>
        )),
      )}
    </MenuMode>
